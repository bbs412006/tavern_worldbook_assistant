...
function walk_index_files(root_dirs) {
    const files = [];
    const extensions = new Set(['.ts', '.tsx', '.js', '.jsx']);
    const stack = root_dirs
    ...
    while (stack.length) {
        ...
        for (const entry of entries) {
            ...
            if (parsed.name !== 'index' || !extensions.has(parsed.ext)) {
                continue;
            }
            files.push(path.relative(process.cwd(), full_path).split(path.sep).join('/'));
        }
    }
    return files;
}
function glob_script_files() {
    const results = [];
    walk_index_files(['示例', 'src'])
        .filter(file => process.env.CI !== 'true' || !fs.readFileSync(path.join(process.cwd(), file)).includes('@no-ci'))
        .forEach(file => {
...
let io;
function watch_tavern_helper(compiler) {
...
let watcher;
const dump = () => {
    exec('pnpm dump', { cwd: process.cwd() });
...
let child_process;
const bundle = () => {
    exec('pnpm sync bundle all', { cwd: process.cwd() });
...
function parse_configuration(entry) {
    const should_obfuscate = fs
        .readFileSync(path.join(process.cwd(), entry.script), 'utf-8')
        .includes('@obfuscate');
    ...
    return (_env, argv) => ({
        ...
        entry: path.join(process.cwd(), entry.script),
        target: 'browserslist',
        output: {
            ...
            devtoolModuleFilenameTemplate: info => {
                ...
                const is_vue_script = resource_path.match(/\.vue$/) &&
            ...
            path: path.join(process.cwd(), 'dist', path.relative(process.cwd(), script_filepath.dir).replace(/^[^\\/]+[\\/]/, '')),
        ...
        plugins: (entry.html === undefined
            ...
            : [
                new HtmlWebpackPlugin({
                    template: path.join(process.cwd(), entry.html),
            ...
            .concat({ apply: watch_tavern_helper }, { apply: schema_dump }, { apply: tavern_sync }, new VueLoaderPlugin(), unpluginAutoImport({
            dts: true,
            dtsMode: 'overwrite',
            imports: [
                'vue',
        ...
        }), new webpack.optimize.LimitChunkCountPlugin({ maxChunks: 1 }), new webpack.DefinePlugin({
            __VUE_OPTIONS_API__: false,
            __VUE_PROD_DEVTOOLS__: process.env.CI !== 'true',
            __VUE_PROD_HYDRATION_MISMATCH_DETAILS__: false,
        }))
            .concat(should_obfuscate
            ? [
        ...
        externals: ({ context, request }, callback) => {
            ...
            if (['vue', 'vue-router'].every(key => request !== key) &&
            ...
            return callback(null, 'module-import ' + (cdn[request] ?? `https://testingcf.jsdelivr.net/npm/${request}/+esm`));
        },
    });
}
