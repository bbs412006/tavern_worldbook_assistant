...
checks = {
    ...
    'cross copy mobile stepper component exists': (root / 'src/worldbook_assistant_build/components/CrossCopyMobileStepper.vue').exists(),
    ...
    'cross copy panel component exists': (root / 'src/worldbook_assistant_build/components/CrossCopyPanel.vue').exists(),
    'app uses cross copy panel for mobile and desktop': app.count('<CrossCopyPanel') >= 2,
    'cross copy panel composes mobile and desktop workspaces': all(token in ((root / 'src/worldbook_assistant_build/components/CrossCopyPanel.vue').read_text(encoding='utf-8') if (root / 'src/worldbook_assistant_build/components/CrossCopyPanel.vue').exists() else '') for token in ['<CrossCopyMobileWorkspace', '<CrossCopyDesktopWorkspace']),
    ...
    'cross copy apply composable exists': (root / 'src/worldbook_assistant_build/composables/useCrossCopyApply.ts').exists(),
    'app imports cross copy apply composable': "from './composables/useCrossCopyApply'" in app and 'useCrossCopyApply' in app,
    'app no longer declares cross copy apply function inline': 'async function applyCrossCopySelection' not in app,
}
